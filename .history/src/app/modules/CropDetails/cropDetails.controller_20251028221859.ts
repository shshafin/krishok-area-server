...
const getCropDetailsByCropIdController = async (
  req: Request,
  res: Response,
) => {
  try {
    const { cropId } = req.params;
    const result = await CropDetailsService.getCropDetailsByCropId(cropId);
    res.status(200).json({
      success: true,
      message: 'Crop details fetched',
      data: result,
    });
  } catch (error) {
    res
...
export const CropDetailsController = {
  ...
  updateCropDetail,
  deleteCropDetail,
  getCropDetailsByCropIdController,
};
