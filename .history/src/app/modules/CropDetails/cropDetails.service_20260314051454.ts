...
const getAllCropDetails = async () => {
  return await CropDetail.find()
    .populate('category', 'englishName banglaName category')
    .sort({ createdAt: -1 });
};

const getSingleCropDetail = async (id: string) => {
  return await CropDetail.findById(id).populate(
    'category',
    'englishName banglaName category',
  );
};
