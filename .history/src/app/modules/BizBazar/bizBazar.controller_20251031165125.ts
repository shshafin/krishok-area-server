...
const deleteBizBazar: RequestHandler = async (req: any, res, next) => {
  try {
    const userId = req.user._id;
    const { id } = req.params;

    const result = await BizBazarServices.deleteBizBazar(id, userId);
    if (!result) {
      return res.status(404).json({
        success: false,
        message: 'Biz Bazar not found or unauthorized',
      });
    }
