...
const getAllCompanies: RequestHandler = async (req, res, next) => {
  try {
    const result = await VideoServices.getAllCompanies();
...
const searchVideoByName: RequestHandler = async (req, res, next) => {
  try {
    const { name } = req.params;
    const result = await VideoServices.searchVideoByName(name);
    res.status(200).json({ success: true, data: result });
  } catch (err) {
    next(err);
  }
